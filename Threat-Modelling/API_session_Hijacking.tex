...
predictable session ID \textquotesingle{}XXX\textquotesingle{}, proTree
    [Legitimate user\\
authenticates and\\
establishes an API connection\\
with session ID \textquotesingle{}XXX\textquotesingle{}]
    [Attacker predicts user\textquotesingle{}s\\
session ID \textquotesingle{}XXX\textquotesingle{}]
    [Attacker authenticates to\\
the API server]
  ]\andN
  [Attacker establishes session\\
with the salon API server using\\
session ID \textquotesingle{}YYY\textquotesingle{} obtained\\
via client side XSS attack, proTree
    [Attacker obtains victim\textquotesingle{}s\\
session ID \textquotesingle{}YYY\textquotesingle{} via\\
client side XSS attack
      [Attacker sends malicious\\
link to the salon API user]
      [Victim salon API user clicks\\
at the malicious link]
      [Victim salon API user\\
is authenticated and has\\
an active session with the\\
salon API using session ID \textquotesingle{}YYY\textquotesingle{}]
    ]\andN
    [Attacker authenticates to\\
the salon API Server]
  ]\andN
  [Use mechanism such as Oauth for the\\
salon API authentication and session\\
management, oppTree
    [Correctly configure Oauth, oppTree]
    [Implement Session Timeouts, oppTree]
    [Require provision of an access\\
token to every salon API requests to\\
protected resources, oppTree]
    [Use functions that generate\\
random session IDs with\\
zero collissions, oppTree]
    [Wrong Oauth configuration, proTree]
  ]
]
\end{forest}
\end{document}
