...
\begin{document}
\begin{forest}
[Impersonate API User, proTree
  [Admin access user\textquotesingle{}s\\
clear text password or\\
static authenticator, proTree
    [Admin access\\
a user\textquotesingle{}s account]
    [Admin resets\\
user\textquotesingle{}s\\
password/static\\
authenticator]
  ]\andN
  [Admin access user\textquotesingle{}s\\
clear text password or\\
static authenticator, proTree
    [Admin receives user\textquotesingle{}s\\
request for password\\
reset]
    [Admin resets user\textquotesingle{}s\\
password/static authenticator]
    [API user sends\\
admin\\
request for\\
password reset]
  ]\andN
  [Make admins send password\\
reset links to users instead of\\
manually resetting password, oppTree
    [Force password change\\
during 1st login]
    [Log all API\textquotesingle{}s\\
authentication request actions]
    [Store authentication logs\\
in security audit file]
  ]
]
\end{forest}
\end{document}
